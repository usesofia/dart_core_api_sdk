import 'package:test/test.dart';
import 'package:dart_api_sdk/dart_api_sdk.dart';

// tests for BankTransactionsControllerGet200Response
void main() {
  final instance = BankTransactionsControllerGet200ResponseBuilder();
  // TODO add properties to the builder and call build()

  group(BankTransactionsControllerGet200Response, () {
    // num pageIndex
    test('to test the property `pageIndex`', () async {
      // TODO
    });

    // num pageSize
    test('to test the property `pageSize`', () async {
      // TODO
    });

    // num totalItems
    test('to test the property `totalItems`', () async {
      // TODO
    });

    // num totalPages
    test('to test the property `totalPages`', () async {
      // TODO
    });

    // BuiltList<BankTransactionEntity> data
    test('to test the property `data`', () async {
      // TODO
    });

  });
}
